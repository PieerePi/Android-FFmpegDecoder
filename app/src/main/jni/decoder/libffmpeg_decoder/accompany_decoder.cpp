...
//	av_register_all();
    ...
    AVCodec *dec;
	stream_index = av_find_best_stream(avFormatContext, AVMEDIA_TYPE_AUDIO, -1, -1, &dec, 0);
...
//		timeBase = av_q2d(audioStream->codec->time_base);
    ...
    if (avCodecContext == NULL) {
        LOGI("Not enough memory to alloc context ");
		avformat_close_input(&avFormatContext);
        return -1;
    }
...
//	}
	// 打开解码器
	result = avcodec_open2(avCodecContext, dec, NULL);
	if (result < 0) {
		LOGI("fail avcodec_open2 result is %d", result);
		avcodec_free_context(&avCodecContext);
		avformat_close_input(&avFormatContext);
		return -1;
	} else {
		LOGI("sucess avcodec_open2 result is %d", result);
	}
	//4、判断是否需要resampler
...
//					duration = av_frame_get_pkt_duration(pAudioFrame) * timeBase;
					position = pAudioFrame->best_effort_timestamp * timeBase;
...
//		avCodecContext = NULL;
	...
	if (NULL != avFormatContext) {
		LOGI("leave AccompanyDecoder::destroy");
		avformat_close_input(&avFormatContext);
	}
//	LOGI("end destroy!!!");
}
